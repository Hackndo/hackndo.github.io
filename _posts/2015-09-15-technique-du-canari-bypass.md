...
!![img]({{ site.baseurl }}assets/uploads/2015/09/canari_ok.png)

!![img]({{ site.baseurl }}assets/uploads/2015/09/canari_ko.png)
...
<pre lang="sh" class="">$ gcc -Wall -m32 -fstack-protector -o canari canari.c</pre>
...
<pre lang="sh" class="">$ ./checksec.sh --file canari
RELRO      STACK CANARY   NX           PIE      RPATH      RUNPATH      FILE
No RELRO   Canary found   NX enabled   No PIE   No RPATH   No RUNPATH   canari</pre>

Essayons alors de provoquer un bête overflow

<pre lang="sh" class="">$ ./canari $(perl -e 'print "A"x100')

AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
*** stack smashing detected ***: ./canari terminated</pre>
...
<pre lang="sh" class="">$ gdb -q canari
...
Bon, nous avons les versions désassemblées de la fonction _main_ et de la fonction _func_. La fonction _main_ ne semble pas avoir été modifiée. En revanche, la fonction _func_ a une fin assez étrange &#8230;

<pre lang="sh" class="">0x080484e0 <+52>: mov eax,DWORD PTR [ebp-0xc]
 0x080484e3 <+55>: xor eax,DWORD PTR gs:0x14
 0x080484ea <+62>: je 0x80484f1 <func+69>
 0x080484ec <+64>: call 0x8048360 <__stack_chk_fail@plt></pre>
...
<pre lang="sh" class="">0x080484e3 <+55>: xor eax,DWORD PTR gs:0x14
 0x080484ea <+62>: je 0x80484f1 <func+69></pre>

comparent ce canari avec la valeur originale. Comme elle n'est pas modifiée, le xor donne la valeur 0 et le jump JE est pris, sautant l'appel à _\_stack\_chk_fail, donc évitant le lancé de tomates.

Maintenant, tentons un buffer overflow

<pre lang="sh" class="">gdb-peda$ r $(perl -e 'print "A"x100')
...

gdb-peda$ i r eax
eax 0x41414141 0x41414141</pre>

Et voilà, nous avons remplacé le canari. Malheur ! Si nous exécutons les quelques instructions qui suivent

<pre lang="sh" class="">gdb-peda$ ni
gdb-peda$ ni
...
=> 0x80484ec <func+64>: call 0x8048360 <__stack_chk_fail@plt>
   ...
   0x80484f3 :    push ebp
   0x80484f4 <main+1>:  mov ebp,esp</pre>

Comme attendu, nous ne prenons pas le saut JE à la ligne _func+62_ et tombons tout droit dans le _call_ à __\_stack\_chk_fail_ qui termine notre programme.
...
![img]({{ site.baseurl }}assets/uploads/2015/09/bf_canari1.png)
...
Une fois cette valeur découverte, il ne reste plus qu'à faire une exploitation de buffer overflow classique. Pour cela, je vous conseille de lire l'article sur [les buffer overflows](http://blog.hackndo.com/buffer-overflow-stack-based/) ou celui sur [le retour à la libc](http://blog.hackndo.com/retour-a-la-libc/)

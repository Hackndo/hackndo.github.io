...
Notons que le PAC est doublement signé. Une première signature via le hash NTLM du compte de service, et une deuxième via le hash NTLM de `krbtgt`. Cependant, lorsque le service reçoit ce ticket, il ne vérifie en général que la première signature. En effet, les comptes de services ayant le privilège `SeTcbPrivilege`, signifiant que ce compte peut agir en tant que partie du système d'exploitation (par exemple le compte local `SYSTEM`), ne vérifient pas la signature du contrôleur de domaine. Pratique, d'un point de vue attaquant ! Ca signifie également que même si le mot de passe du compte `krbtgt` est changé, les Silver Tickets pourront toujours fonctionner.
